...
struct CustomProgressView<Label, CurrentValueLabel>: PrimitiveView, UnaryView, View where Label: View, CurrentValueLabel: View {
    ...
    nonisolated static func _makeView(
        ...
        let child = Child(
            baseValue: view.value[offset: { .of(&$0.value) }],
            label: view.value[offset: { .of(&$0.label) }],
            currentValueLabel: view.value[offset: { .of(&$0.currentValueLabel) }]
        )
        var outputs = Child.Value._makeView(
            view: _GraphValue(child),
            inputs: inputs
        )
        if inputs.preferences.containsWidgetAuxiliaryViewMetadata() {
            outputs.preferences[WidgetAuxiliaryViewMetadata.Key.self] = nil
                    ...
                    WidgetMetadataWriter(
                        ...
                        labelPref: Attribute(
                            LazyWidgetAuxiliaryMetadataTextImage(
                                content: view.value[offset: { .of(&$0.label) }],
                                inputs: inputs
                            )
                        ),
                        currentValueLabelPref: Attribute(
                            LazyWidgetAuxiliaryMetadataTextImage(
                                content: view.value[offset: { .of(&$0.currentValueLabel) }],
                                inputs: inputs
                            )
                        ),
    ...
    private struct WidgetMetadataWriter: Rule {
        ...
        var value: WidgetAuxiliaryViewMetadata? {
            let kind = WidgetAuxiliaryViewMetadata.Progress.Kind(baseValue)
